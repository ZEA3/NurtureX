...
const PAGE_SIZE = 12
const EMPTY_FORM = {
  mother_id: '', doctor_id: '', name: '', date_of_birth: '', gender: 'male',
...
export default function AdminInfants() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const { rows, total } = await infantService.list({ search, status, page, pageSize: PAGE_SIZE })
    ...
    <>
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-zinc-500 mt-1">All infants in the platform.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {rows.map(i => (
            <InfantCard key={i.id} infant={i} onClick={() => navigate(`/admin/infants/${i.id}`)} />
          ))}
        </div>
      )}
      ...
      <Modal
        ...
        title="Register new infant"
        size="lg"
        footer={
          <>
            <Button variant="secondary" onClick={() => setModalOpen(false)} disabled={saving}>Cancel</Button>
            <Button onClick={submit} loading={saving}>Add infant</Button>
          </>
        }
      >
