...
//
// Doctor's view of their own patients. RLS scopes to doctor_id = auth.uid()
// so even without a client filter they only see their assigned mothers.

import { useEffect, useState, useCallback } from 'react'
import { Plus, Search, Pencil, Trash2, Users, RefreshCw, AlertCircle, Phone, Mail, MessageSquare } from 'lucide-react'
...
const EMPTY_FORM = {
  ...
  blood_type: '', notes: '', status: 'active',
}
...
export default function DoctorPatients() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { rows, total } = await patientService.list({
        search, status, doctorId: user.id, page, pageSize: PAGE_SIZE,
      })
  ...
  const openEdit   = (p) => {
    ...
    setForm({
      ...
      age: p.age ?? '', due_date: p.due_date ?? '', blood_type: p.blood_type ?? '',
  ...
  const submit = async (e) => {
    ...
    setSaving(true)
    try {
      const payload = { ...form, doctor_id: user.id }
      if (editing) {
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">My patients</h1>
          <p className="text-sm text-slate-500 dark:text-zinc-500 mt-1">Mothers under your care.</p>
        </div>
            ...
            {['all', 'active', 'discharged', 'archived'].map(s => (
              <button key={s} onClick={() => setStatus(s)}
                className={'px-3 h-9 rounded-full text-xs font-semibold capitalize transition border ' +
                  (status === s ? 'bg-brand-700 text-white border-brand-700 dark:bg-white dark:text-black dark:border-white' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50 dark:bg-zinc-900 dark:text-zinc-400 dark:border-zinc-800 dark:hover:bg-zinc-800')}>{s}</button>
            ))}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-zinc-800">
              ...
              : rows.length === 0 ? (
                <tr><td colSpan={5}>
                  <EmptyState icon={Users} title={search ? 'No matches' : 'No patients yet'} description={search ? 'Try a different search.' : 'Click "Add patient" to start.'} action={!search && <Button onClick={openCreate}><Plus size={14} /> Add patient</Button>} />
                </td></tr>
              ) : rows.map(p => (
                <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-zinc-950 transition">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-[11px] text-slate-500 dark:text-zinc-500">{p.age ? `${p.age} yrs` : '—'} · {p.blood_type || '—'}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-5 py-3.5 hidden md:table-cell text-xs text-slate-500 dark:text-zinc-500">{p.due_date ? new Date(p.due_date).toLocaleDateString() : '—'}</td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1.5 justify-end">
                      <Button variant="ghost" size="xs" onClick={() => navigate(`/doctor/messages?patient=${p.id}`)} title="Message"><MessageSquare size={13} /></Button>
                      <Button variant="ghost" size="xs" onClick={() => openEdit(p)}><Pencil size={13} /></Button>
                      <Button variant="danger" size="xs" onClick={() => setDeleteTarget(p)}><Trash2 size={13} /></Button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {/* Modal — uses doctor_id auto-assigned to current user */}
      <Modal open={modalOpen} onClose={closeModal} title={editing ? `Edit ${editing.full_name}` : 'Add patient'} size="lg"
        footer={<><Button variant="secondary" onClick={closeModal} disabled={saving}>Cancel</Button><Button onClick={submit} loading={saving}>{editing ? 'Save changes' : 'Add patient'}</Button></>}>
        <form onSubmit={submit}>
          ...
          <Field label="Full name" required><Input value={form.full_name} onChange={(e) => setForm(f => ({ ...f, full_name: e.target.value }))} disabled={saving} /></Field>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Field label="Email"><Input type="email" value={form.email} onChange={(e) => setForm(f => ({ ...f, email: e.target.value }))} disabled={saving} /></Field>
            <Field label="Phone"><Input type="tel" value={form.phone} onChange={(e) => setForm(f => ({ ...f, phone: e.target.value }))} disabled={saving} /></Field>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <Field label="Age"><Input type="number" min={0} value={form.age} onChange={(e) => setForm(f => ({ ...f, age: e.target.value }))} disabled={saving} /></Field>
            <Field label="Due date"><Input type="date" value={form.due_date} onChange={(e) => setForm(f => ({ ...f, due_date: e.target.value }))} disabled={saving} /></Field>
            <Field label="Blood type"><Input value={form.blood_type} onChange={(e) => setForm(f => ({ ...f, blood_type: e.target.value }))} disabled={saving} /></Field>
          </div>
          <Field label="Status">
            <Select value={form.status} onChange={(e) => setForm(f => ({ ...f, status: e.target.value }))} disabled={saving}>
              <option value="active">Active</option><option value="discharged">Discharged</option><option value="archived">Archived</option>
            </Select>
          </Field>
        </form>
      </Modal>

      <Modal open={!!deleteTarget} onClose={() => !deleting && setDeleteTarget(null)} size="sm" title="Delete patient?"
        footer={<><Button variant="secondary" onClick={() => setDeleteTarget(null)} disabled={deleting}>Cancel</Button><Button variant="danger" onClick={confirmDelete} loading={deleting}>Delete</Button></>}>
        <p className="text-sm text-slate-700 dark:text-zinc-300 leading-relaxed">
          <span className="font-semibold">{deleteTarget?.full_name}</span> and all associated infant records will be permanently deleted.
        </p>
      </Modal>
    </>
  )
